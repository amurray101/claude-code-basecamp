  ...
  },
  {
    ...
    content: [
      ...
      { type: "stats", heading: "Fast Stats", items: [
        { stat: "72.7%+", label: "SWE-bench Verified score (Sonnet 4) — the industry standard for real-world coding ability" },
        { stat: "200K–1M", label: "Token context window — Claude reads entire codebases, not just open files" },
        { stat: "2×", label: "Code output per engineer internally at Anthropic after adopting Claude Code" },
        { stat: "46%", label: "'Most loved' among developers in 2026 surveys — highest of any AI coding tool" },
        { stat: "~70%", label: "Of Fortune 100 companies use Claude across their organization" },
        { stat: "7 min", label: "Median time to first agentic task — install to working code in a single session" },
      ]},
  ...
  },
  {
    ...
    content: [
      ...
      { type: "overview", heading: "What we'll cover", items: [
        ...
        { label: "Permissions", desc: "Tiered control model from interactive to headless CI/CD" },
      ...
      { type: "outcomes", items: [
        ...
        "Explain the three permission modes and when each is appropriate for a given deployment",
      ...
      { type: "text", value: "Hooks let you run custom scripts before or after Claude Code takes actions. Pre-commit hooks can enforce linting. Post-edit hooks can run tests. This is the answer to every security-conscious customer who asks, \"How do I control what Claude does?\"", simple: "Hooks are automatic checkpoints. You set up scripts that run before or after Claude does something. For example, before Claude saves a code change, a hook can check for style errors (\"linting\"). After Claude edits a file, a hook can run the relevant tests. This is the answer when a security-focused customer asks how they control what Claude does." },
      ...
      { type: "heading", value: "Permissions model", simple: "Three levels of control over what Claude can do" },
      { type: "text", value: "Claude Code has a tiered permission system. In the default interactive mode, it asks before editing files, running commands, or making network requests. In headless mode (CI/CD), permissions are explicitly configured. This is a critical differentiator for enterprise customers — it's the answer to 'How do we deploy this safely at scale?'", simple: "Claude Code has three levels of permission, from most supervised to fully automatic. By default, it asks before doing anything significant. You can also let it run on its own within rules you set. For large companies, this tiered approach answers the key question: how do we use this safely across hundreds of developers?" },
      { type: "values", items: [
        { title: "Interactive mode", desc: "Claude asks permission before destructive actions. User approves each step. Good for learning and exploration.", simpleDesc: "Claude pauses and asks before making changes, running commands, or accessing the network. You approve each step. Best when learning the tool or exploring a new codebase." },
        { title: "Auto-accept mode", desc: "Claude executes without asking, within configured boundaries. Good for experienced users who trust their CLAUDE.md setup.", simpleDesc: "Claude works on its own without pausing, but only within boundaries you have set. Like giving a trusted colleague access with clear rules about what they can and cannot touch." },
        { title: "Headless mode", desc: "No human in the loop — runs in CI/CD pipelines. Permissions set via config files. The enterprise deployment pattern.", simpleDesc: "No human watches in real time. Claude runs inside automated build-and-deploy pipelines (CI/CD). Permissions are locked down in config files. This is how large companies deploy Claude Code at scale." },
      ]},
...
function ContentBlock({ block, idx, onReflect, simplified }) {
  ...
  if (block.type === "placeholder") return (
    <div style={{ margin: "20px 0", padding: "20px 24px", background: C.blue + "06", borderRadius: 10, border: `1px dashed ${C.blue}40`, ...st.fadeUp, animationDelay: delay }}>
      <div style={{ fontFamily: "var(--mono)", fontSize: 10, letterSpacing: 2, color: C.blue, textTransform: "uppercase", marginBottom: 8 }}>Go deeper — content needed</div>
              ...
              <div style={{ marginBottom: 20, ...st.fadeUp, animationDelay: "0.3s" }}>
                {mod.gaps.map((gap, gi) => (
                  <div key={gi} style={{ margin: "0 0 12px", padding: "18px 22px", background: C.blue + "06", borderRadius: 10, border: `1px dashed ${C.blue}40` }}>
                    <div style={{ fontFamily: "var(--mono)", fontSize: 9, letterSpacing: 1.5, color: C.blue, textTransform: "uppercase", marginBottom: 6 }}>Go deeper — content needed</div>
